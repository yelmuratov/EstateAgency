...
export const commercialFormSchema = z.object({
  ...
  square_area: z.number().int().positive('Площадь должна быть положительным числом'),
  floor_number: z.number().int().positive('Этаж должен быть положительным числом'),
  location: z.enum(['business_center', 'administrative_building', 'residential_building', 'cottage', 'shopping_mall', 'industrial_zone', 'market', 'detached_building'], { required_error: 'Выберите расположение' }),
  furnished: z.boolean().optional().default(true),
  ...
  agent_percent: z.number().int().min(0, 'Процент агента не может быть отрицательным').max(100, 'Процент агента не может быть больше 100'),
